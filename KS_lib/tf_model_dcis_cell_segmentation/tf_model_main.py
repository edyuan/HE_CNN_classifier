...
from KS_lib.tf_model_dcis_cell_segmentation import tf_model_test
import numpy as np

def main(nth_fold,mode,flags,test_images_list, result_path,dcis_segmentation_result_path, igpu):
    ...
    if mode == 'test_model':
        ...
        filename_list = [[x[0]] for x in test_images_list]
        filename_list_he = [[x[1]] for x in test_images_list]

        tf_model_test.test(object_folder, model_path, filename_list, filename_list_he, flags,result_path,dcis_segmentation_result_path, igpu)
